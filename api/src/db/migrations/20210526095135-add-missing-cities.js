  ...
  up: async (queryInterface) =>
    Promise.all([
      ...
      queryInterface.sequelize.query(
        `INSERT INTO cities (${fields}, nom_comm, population, insee_com) (SELECT ${fields}, "LYON", ${POPULATION_LYON}, "${CODE_INSEE_LYON}" from cities WHERE insee_com = "${CODE_INSEE_LYON_FIRST_DISTRICT}" LIMIT 1)`
      ),
      queryInterface.sequelize.query(
        `INSERT INTO cities (${fields}, nom_comm, population, insee_com) (SELECT ${fields}, "PARIS", ${POPULATION_PARIS}, "${CODE_INSEE_PARIS}" from cities WHERE insee_com = "${CODE_INSEE_PARIS_FIRST_DISTRICT}" LIMIT 1)`
      ),
      queryInterface.sequelize.query(
        `INSERT INTO cities (${fields}, nom_comm, population, insee_com) (SELECT ${fields}, "MARSEILLE", ${POPULATION_MARSEILLE}, "${CODE_INSEE_MARSEILLE}" from cities WHERE insee_com = "${CODE_INSEE_MARSEILLE_FIRST_DISTRICT}" LIMIT 1)`
      ),
