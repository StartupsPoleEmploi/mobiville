  ...
  try {
    ...
    const formattedTemperatures = temperatures
      ...
      .map(({ date_obs, code_insee_departement, temperature }) => {
        ...
        let seasonIndex = seasonsDate.indexOf(seasonsDate.find(seasonsDate => date < seasonsDate))
    ...
    const result = Object.values(formattedTemperatures)
      .map(departementData => {
        const seasonNumber = [
...
console.log(
  'Starting synchronization for departements temperatures.'
)
doSync()
